...
if __name__ == "__main__":
    ...
    outputWWl_dy0j[args.observable]=scale_xs(outputWWl_dy0j[args.observable],lumi,outputWWl_dy0j['sumw'])
    hist_b_nom=outputWWl_b[args.observable].group("dataset",hist.Cat("plotgroup", "plotgroup"),merge_map['hww_tem'])
    hist_b_dy=outputWWl_b[args.observable].group("dataset",hist.Cat("plotgroup", "plotgroup"),merge_map['hww_tem_jetbin'])
    hist_dy0j_jetbin=outputWWl_dy0j[args.observable].group("dataset",hist.Cat("plotgroup", "plotgroup"),merge_map['hww_tem_jetbin'])
    hist_dy_jetbin=outputWWl_dy[args.observable].group("dataset",hist.Cat("plotgroup", "plotgroup"),merge_map['hww_tem_jetbin'])
    hist_dynlo_jetbin=outputWWl_dynlo[args.observable].group("dataset",hist.Cat("plotgroup", "plotgroup"),merge_map['hww_tem_jetbin'])    
    hist_dynlo_zptbin=outputWWl_dynlo[args.observable].group("dataset",hist.Cat("plotgroup", "plotgroup"),merge_map['hww_tem_zptbin'])   
    hist_dynlo_ptbin=outputWWl_dynlo[args.observable].group("dataset",hist.Cat("plotgroup", "plotgroup"),merge_map['hww_tem_ptbin'])    
    ...
    for region in ['SR','SR2','top_CR', 'DY_CR']:
        ...
        for flav in ['ee','mumu','emu']:
            ...
            output_bkg_nom = hist_b_nom.integrate("lepflav",flav).integrate("region",region).sum('flav')
            output_bkg_jetbin= hist_b_dy.integrate("lepflav",flav).integrate("region",region).sum('flav').add(hist_dy_jetbin.integrate("lepflav",flav).integrate("region",region).sum('flav'))
            output_bkg_jetbin = output_bkg_jetbin.add(hist_dy0j_jetbin.integrate("lepflav",flav).integrate("region",region).sum('flav'))
            # print(hist_dy0j_jetbin.integrate("lepflav",flav).integrate("region",region).sum('flav').values()[()])
            output_bkg_jetbin = output_bkg_jetbin.add(hist_dynlo_jetbin.integrate("lepflav",flav).integrate("region",region).sum('flav'))
            output_bkg_zptbin= hist_b_dy.integrate("lepflav",flav).integrate("region",region).sum('flav').add(hist_dy_jetbin.integrate("lepflav",flav).integrate("region",region).sum('flav'))
            output_bkg_zptbin= output_bkg_zptbin.add(hist_dy0j_jetbin.integrate("lepflav",flav).integrate("region",region).sum('flav'))
            output_bkg_zptbin= output_bkg_zptbin.add(hist_dynlo_zptbin.integrate("lepflav",flav).integrate("region",region).sum('flav'))
            output_bkg_ptbin= hist_b_dy.integrate("lepflav",flav).integrate("region",region).sum('flav').add(hist_dy_jetbin.integrate("lepflav",flav).integrate("region",region).sum('flav'))
            output_bkg_ptbin = output_bkg_ptbin.add(hist_dy0j_jetbin.integrate("lepflav",flav).integrate("region",region).sum('flav'))
            output_bkg_ptbin= output_bkg_ptbin.add(hist_dynlo_ptbin.integrate("lepflav",flav).integrate("region",region).sum('flav'))
            ...
            # ax.semilogy()
            hep.mpl_magic(ax=ax)
            rax = hist.plotratio(num=outputWWl_data[args.observable].integrate("lepflav",flav).integrate("region",region).sum('flav').integrate('plotgroup',f'data_{flav}'),
                                                    denom=output_bkg_nom.sum('plotgroup'),
                                                            ax=rax,
                                                            error_opts={},
                                                            denom_fill_opts={'alpha': 0.1,'facecolor':(31/256, 119/256, 180/256,0.5)},
                                                            unc='num',
                                                            clear=False
                                                            )
            hist.plotratio(num=outputWWl_data[args.observable].integrate("lepflav",flav).integrate("region",region).sum('flav').integrate('plotgroup',f'data_{flav}'),
                                                        denom=output_bkg_jetbin.sum('plotgroup'),
                                                            ax=rax,
                                                            error_opts={},
                                                            denom_fill_opts={'alpha': 0.1,'facecolor':(255/256, 127/256, 14/256,0.5)},
                                                            unc='num',
                                                        clear=False
                                                            )
            hist.plotratio(num=outputWWl_data[args.observable].integrate("lepflav",flav).integrate("region",region).sum('flav').integrate('plotgroup',f'data_{flav}'),
                                                        denom=output_bkg_zptbin.sum('plotgroup'),
                                                            ax=rax,
                                                            error_opts={},
                                                            denom_fill_opts={'alpha': 0.1,'facecolor':(44/256, 160/256, 44/256,0.5)},
                                                            unc='num',
                                                        clear=False
                                                            )
            # hist.plotratio(num=outputWWl_data[args.observable].integrate("lepflav",flav).integrate("region",region).sum('flav').integrate('plotgroup',f'data_{flav}'),
